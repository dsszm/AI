...
router.post('/chat', async (req, res) => {
    ...
    if (useAdminKey)
        recordAccess(sid, clientIp, 'anonymous');
    // 持久化用户消息
    ...
    try {
        await streamChat({ apiKey, messages, model }, {
            onToken: (token) => {
...
router.post('/chat/demo', async (req, res) => {
    ...
    const lastUserMsg = [...messages].reverse().find((m) => m.role === 'user');
    if (lastUserMsg)
        saveMessage(sid, 'user', lastUserMsg.content, 'qwen', lastUserMsg.images);
    res.writeHead(200, {
        'Content-Type': 'text/event-stream',
        'Cache-Control': 'no-cache, no-transform',
        Connection: 'keep-alive',
...
function truncateTitle(messages) {
