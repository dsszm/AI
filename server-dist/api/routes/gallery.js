...
// 通过URL添加图片(管理员)
router.post('/gallery/url', requireAdmin, async (req, res) => {
    ...
    db.prepare('INSERT INTO gallery_item (id, type, url, thumbnail, title, category_id) VALUES (?, ?, ?, ?, ?, ?)').run(id, 'image', url.trim(), url.trim(), safeTitle, categoryId || null);
    const item = db
...
// 上传本地图片(管理员)
router.post('/gallery/upload', requireAdmin, upload.single('image'), (req, res) => {
    const file = req.file;
    ...
    db.prepare('INSERT INTO gallery_item (id, type, url, thumbnail, title, category_id) VALUES (?, ?, ?, ?, ?, ?)').run(id, 'image', filePath, filePath, safeTitle, categoryId || null);
    const item = db
        .prepare('SELECT id, type, url, thumbnail, title, category_id as categoryId FROM gallery_item WHERE id = ?')
        .get(id);
    res.json({ success: true, data: item });
...
// 分配内容到分类
router.post('/gallery/:id/category', requireAdmin, (req, res) => {
    const { id } = req.params;
