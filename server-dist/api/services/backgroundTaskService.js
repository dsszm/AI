...
function logTask(name, status, message, duration) {
    const result = {
        name,
        status,
        message,
...
function taskDbOptimize() {
    ...
    try {
       ...
       FROM usage_log`).get();
        logTask('数据库优化', 'success', `VACUUM完成，使用量记录: ${stats.total}条(对话${stats.chat_count}/生图${stats.image_count})`, Date.now() - start);
    }
    catch (err) {
...
function taskCleanOrphanFiles() {
    ...
    try {
        ...
        const db = getDb();
        const referencedUrls = db.prepare('SELECT url FROM gallery_item WHERE url LIKE ?').all('/uploads/%');
 ...
 */
function taskUsageAnalysis() {
    const start = Date.now();
    try {
        const db = getDb();
        // 统计最近7天每日使用量
        const dailyStats = db.prepare(`SELECT
       ...
       ORDER BY date DESC`).all();
        // 计算每小时的使用趋势
        const hourlyStats = db.prepare(`SELECT
       ...
       ORDER BY hour`).all();
        ...
        logTask('使用量分析', 'success', `7天${dailyStats.length}条日统计, 24h${hourlyStats.length}条小时统计, 均值${avg.toFixed(1)}±${stddev.toFixed(1)}, 范围[${min}, ${max}]`, Date.now() - start);
    }
    catch (err) {
...
function taskHealthCheck() {
    ...
    try {
        ...
        logTask('健康检查', 'success', `RSS=${memMB}MB, Heap=${heapUsed}/${heapTotal}MB, 运行${uptimeHours.toFixed(1)}h`, Date.now() - start);
    }
    catch (err) {
 ...
 * 手动触发所有任务
 */
export function runAllTasksNow() {
    for (const task of scheduledTasks) {
