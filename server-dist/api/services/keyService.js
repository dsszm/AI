 ...
 * 保存某模型的 API Key
 */
export function saveKey(model, apiKey, persist) {
    if (persist) {
        const db = getDb();
        const encrypted = encrypt(apiKey);
        db.prepare(`INSERT INTO key_config (model, encrypted_key) VALUES (?, ?)
       ON CONFLICT(model) DO UPDATE SET encrypted_key = excluded.encrypted_key, created_at = CURRENT_TIMESTAMP`).run(model, encrypted);
        // 同步写入会话缓存
...
export function getApiKey(model) {
    ...
    // 2. 加密存储
    const db = getDb();
    const row = db.prepare('SELECT encrypted_key FROM key_config WHERE model = ?').get(model);
    if (row) {
...
function isConfigured(model, db) {
    if (sessionKeys.has(model))
        return true;
    const row = db.prepare('SELECT 1 FROM key_config WHERE model = ?').get(model);
    if (row)
        return true;
    // 通义千问有管理员默认 Key 也算已配置
    if (model === 'qwen' && (process.env.QWEN_API_KEY || process.env.DASHSCOPE_API_KEY))
        return true;
    return false;
}
/**
 * 删除某模型 Key
 */
export function deleteKey(model) {
