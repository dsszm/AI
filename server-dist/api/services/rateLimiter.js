 ...
 */
export function recordAccess(sessionId, ip, userName) {
    const db = getDb();
    const now = Date.now();
    const windowMs = 60000; // 1 分钟窗口
    const threshold = 10; // 1 分钟内超过 10 次标记为异常
    db.prepare(`INSERT INTO user_meta (session_id, ip, user_name, access_count, flagged, last_request_at)
     VALUES (?, ?, ?, 1, 0, ?)
     ON CONFLICT(session_id) DO UPDATE SET
       access_count = access_count + 1,
       last_request_at = excluded.last_request_at`).run(sessionId, ip, userName, new Date().toISOString());
    // 检查窗口内是否高频
    const recent = db
        .prepare(`SELECT last_request_at FROM user_meta WHERE session_id = ?`)
        .get(sessionId);
    if (recent) {
